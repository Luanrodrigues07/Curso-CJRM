...
const impares = randomNumbers.filter(randomNumber => randomNumber % 2 === 1)
console.log(impares)
/*
  02
...
const numbers501 = crazyNumbers.reduce((accumulator, crazyNumber) => {
    if (crazyNumber < 501) {
        accumulator += 1
    }
    return accumulator
...
const numelevado = numbers.map((number) => number ** 2)
...
/*
  ...
  - Nome 3
*/
const gameName = cart.reduce((accumulator, product) => {
...
const years2000 = tarantinoMovies.filter(tarantinoMovie => tarantinoMovie.release < 2000)
console.log(years2000)
/*
  06
...
const nameSerie = tvShows.map(tvShow => tvShow.name)
