...
const getArray = array = array.map(item => item)

const names = ['Caio', 'André', 'Dário']

const namesCopy = getArray(names).sort()
...
const numbersCopy = getArray(numbers)

numbersCopy.sort(((numbers1, numbers2) => numbers1 - numbers2))
...
const peopleCopy = getArray(people)
peopleCopy.sort()
...
const cooked = ingredients.reduce((acc, item, index, array) => {

  const correct = /a$/.test(item) ? 'cozida' : 'cozido'

  if (index === array.length - 1) {
      return acc + `${item} ${correct},`
  }
}, '')
...
.reduce((acc, item) => acc + `<li>${item.title}</li>`, '')
